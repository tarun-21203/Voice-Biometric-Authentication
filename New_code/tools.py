...
def eval_net(net, device, enroll_dict, test_dict):
    """
    Evaluate the model using the provided enroll and test dictionaries.

    Args:
        net: Trained model.
        device: PyTorch device (CPU/GPU).
        enroll_dict: Enrollment dictionary with speaker IDs as keys and audio paths as values.
        test_dict: Test dictionary with speaker IDs as keys and audio paths as values.

    Returns:
        EER: Equal Error Rate.
        minDCF: Minimum Detection Cost Function.
    """
    # Set up the model for embedding extraction
    net.aug = False
    net.output_embedding = True
    net.to(device)
    net.eval()

    # Step 1: Generate embeddings for enroll dictionary
    embed_dict = {}
    for speaker_id, file_list in enroll_dict.items():
        embeddings = []
        for file_path in file_list:
            embedding = get_embedding(net, file_path, device)
            embeddings.append(embedding)
        # Average embeddings for each speaker
        embed_dict[speaker_id] = torch.mean(torch.stack(embeddings), dim=0)

    # Step 2: Evaluate embeddings using the test dictionary
    score_list = []
    labels = []
    for speaker_id, file_list in test_dict.items():
        for file_path in file_list:
            # Get test embedding
            test_embedding = get_embedding(net, file_path, device)

            # Compute similarity scores with all enroll embeddings
            for enroll_id, enroll_embedding in embed_dict.items():
                score = torch.matmul(test_embedding, enroll_embedding.T).item()
                score_list.append(score)
                labels.append(1 if speaker_id == enroll_id else 0)

    # Step 3: Compute metrics (EER and minDCF)
