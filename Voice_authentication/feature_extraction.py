...
def extract_features(audio_file, sample_rate=None):
    """Extract MFCC features from an audio file or raw waveform.

    Accepts either:
      - `audio_file` as a path/file-like (usual case)
      - `audio_file` as a 1-D NumPy waveform and `sample_rate` provided

    Returns: features as shape (TIME, MFCC)
    """
    ...
    features = librosa.feature.mfcc(y=waveform, sr=16000, n_mfcc=myconfig.N_MFCC)
